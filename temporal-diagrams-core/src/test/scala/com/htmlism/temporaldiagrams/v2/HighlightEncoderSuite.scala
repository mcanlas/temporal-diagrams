...
object HighlightEncoderSuite extends FunSuite {
  test("A diagram encoder can encode") {
    expect.eql(NonEmptyList.one(Component("amazon ec2: abc")), Amazon.Ec2.ec2Encoder.encode(Amazon.Ec2("abc")))
  }

  test("A diagram encoder can encode with highlights") {
    expect.eql(
      NonEmptyList.one(Component("amazon ec2: abc true")),
      Amazon
        .Ec2
        .ec2Encoder
  ...
  test("A diagram encoder is contravariant") {
    ...
    expect.eql(NonEmptyList.one(Component("amazon ec2: twin twin")), repeatEncoder.encode("twin"))
  }
}
